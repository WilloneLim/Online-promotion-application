...
$promo_img = $promo_name = $promo_desp = "";
$cust_id = $promo_id = $tran_sale = $tran_date = $msg = "";
// define variables and set to empty values
if(isset($_GET['id'])){
    $promo_id = $_GET['id'];

    $sqla = "SELECT * FROM promotion WHERE promo_id =".$_GET['id'];
            $resulta = $conn->query($sqla);

            if ($resulta->num_rows > 0) {
                // output data of each row
                while($rowa = $resulta->fetch_assoc()) {
                    $promo_img = $rowa['promo_img'];
                    $promo_name = $rowa['promo_title'];
                    $promo_desp = $rowa['promo_desc'];
                }
            }
    ...
    if($tran_sale==""){
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            if ($resulta->num_rows > 0) {
                // output data of each row
                while($rowa = $resulta->fetch_assoc()) {
                    $pro_id = $rowa['promoter_id'];
                }
            }
            header("Location: promoter.php?id=".$pro_id);
...
<body id ="main">
    ...
    <div class="container">
        ...
        <div class="col-md-6 border-right">
            <div class="row m-2">
            <img src="images/<?php echo $promo_img; ?>" class="col-md-12 h-100">
<!--            <h3>Please Enter Sale Amount:</h3>-->
            <h4 class="col-md-12 mt-3"><?php echo $promo_name; ?></h4>
            <h6 class="col-md-12" id="description"><?php echo $promo_desp; ?></h6>
            </div>
        </div>
        <div class="col-md-6">
            <div class="row m-2">
...
<!--            <h3>Please Enter Sale Amount:</h3>-->
                <div class="col-md-5 col-sm-8">
                    <h6 class="p-0 mt-3">Customer Username:<span class="text-muted font-weight-normal">To Be Implemented</span></h6>
                    <h6 class="p-0 mt-3">Past transations: <span class="text-muted font-weight-normal">To Be Implemented</span></h6>
